...
class TechnicalScore(Base):
    ...
    technical_strength_score: Mapped[Decimal | None] = mapped_column(Numeric)
    setup_quality_score: Mapped[Decimal | None] = mapped_column(Numeric)
    entry_quality_score: Mapped[Decimal | None] = mapped_column(Numeric)
    technical_composite_score: Mapped[Decimal | None] = mapped_column(Numeric)
    confidence_adjusted_score: Mapped[Decimal | None] = mapped_column(Numeric)
    leadership_v5_score: Mapped[Decimal | None] = mapped_column(Numeric)
    residual_momentum_score: Mapped[Decimal | None] = mapped_column(Numeric)
    trigger_distance_atr: Mapped[Decimal | None] = mapped_column(Numeric)
    stop_distance_atr: Mapped[Decimal | None] = mapped_column(Numeric)
    stage_modifier: Mapped[Decimal | None] = mapped_column(Numeric)
    setup_type: Mapped[str | None] = mapped_column(String(64))
    sector_benchmark_symbol: Mapped[str | None] = mapped_column(String(32))
    v5_debug_json: Mapped[dict[str, Any] | None] = mapped_column(JSONB)
