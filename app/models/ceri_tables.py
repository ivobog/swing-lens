...
class CeriIngestionRun(Base):
    ...
    deployment_identity_json: Mapped[dict[str, Any] | None] = mapped_column(JSONB)
...
class CeriEarningsActual(Base):
    ...
    acquisition_policy: Mapped[str | None] = mapped_column(String(32))
    provider_consensus_semantics: Mapped[str | None] = mapped_column(Text)
...
class CeriGuidanceEvent(Base):
    ...
    quality_warnings_json: Mapped[list[str] | None] = mapped_column(JSONB)
    accepted_for_scoring: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    ...
    clean_evidence_excerpt_hash: Mapped[str | None] = mapped_column(String(128))
    processor_signature: Mapped[str | None] = mapped_column(Text)
    normalization_version: Mapped[str | None] = mapped_column(Text)
...
class CeriRevisionFeature(Base):
    ...
    baseline_origin: Mapped[str | None] = mapped_column(String(64))
    comparison_mode: Mapped[str | None] = mapped_column(String(64))
    current_source_record_id: Mapped[int | None] = mapped_column(
        ForeignKey("ceri_source_records.id", ondelete="SET NULL")
    )
    baseline_source_record_id: Mapped[int | None] = mapped_column(
        ForeignKey("ceri_source_records.id", ondelete="SET NULL")
    )
    provider_retrospective_source_record_id: Mapped[int | None] = mapped_column(
        ForeignKey("ceri_source_records.id", ondelete="SET NULL")
    )
    known_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
    reference_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class CeriPriceResponseFeature(Base):
    ...
    reaction_policy_version: Mapped[str | None] = mapped_column(Text)
