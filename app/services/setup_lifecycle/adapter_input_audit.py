...
_SIGNALS: dict[str, tuple[str, str, str, str, str, bool, str]] = {
    ...
    "distance_to_pivot_pct": (
        "completed-close distance from the family-specific trigger reference",
        "SRS FR-027; SDD 6.1/8 family readiness",
        "PriceBar + family-specific canonical trigger evidence",
        "derived from close_price and normalized trigger reference",
    ...
    "trigger_price": (
        ...
        "SDD 8.3/8.4",
        "RawCompanyRow / TechnicalScore / PriceBar",
        "family-specific trigger_reference source lineage",
        "_trigger_reference",
        False,
    ...
    for adapter, keys in _ADAPTER_KEYS.items():
        for key in keys:
            ...
            if key == "close_trigger_cross":
                history = (
                    "consecutive prior canonical true values derive hold/follow-through sessions"
                )
