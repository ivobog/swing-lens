...
class CeriFeatureRebuildService:
    ...
    def prepare_batch(
        ...
        estimates = load(
            CeriEstimateSnapshot,
            select(CeriEstimateSnapshot).where(CeriEstimateSnapshot.company_id.in_(company_ids)),
        )
        earnings = load(
            CeriEarningsActual,
        ...
        source_ids = {
            source_id
            for row in [*estimates, *earnings, *guidance, *revisions]
            for source_id in (
        ...
        revision_features = load(
            CeriRevisionFeature,
            select(CeriRevisionFeature).where(
        ...
        derived_features = load(
            CeriDerivedFeature,
            select(CeriDerivedFeature).where(
        ...
        capabilities = CeriCapabilityMatrixService().build(
            ...
            earnings_company_ids={
                row.company_id
                for row in earnings
                if row.actual_value is not None and row.event_kind in (None, "REPORTED")
            },
            guidance_company_ids={
                row.company_id for row in guidance if row.accepted_for_scoring is True
            },
            catalyst_company_ids={row.company_id for row in events if row.id in eligible_event_ids},
        )
        ...
        return CeriFeatureBatchContext(
            ...
            {_state_key(row): row for row in states},
            bars_by_ticker,
            capabilities,
            pit,
            int((perf_counter() - started) * 1000),
    ...
    def rebuild(
        ...
        *,
        ...
        batch_context: CeriFeatureBatchContext | None = None,
        ...
        if own_context:
            return _replace_result(
                merged,
                load_context_ms=context.load_context_ms,
                sql_select_count=context.select_count,
                rows_loaded=dict(context.rows_loaded),
                family_query_counts={
                    "estimates": 1,
                    "earnings": 1,
                    "guidance": 1,
    ...
    def _rebuild_company(
        ...
        *,
        ...
        revision_service = (
            CeriRevisionFeatureService(config=self.config, query=context.point_in_time_query)
            if isinstance(self.revisions, CeriRevisionFeatureService)
            else self.revisions
        )
        ...
        for metric in (metric.value for metric in self.config.metrics.required):
            for period_slot in self.config.metrics.period_types:
                ...
                revision_rows.extend(
                    ...
                    if not request.from_session or row.as_of_session >= request.from_session
                )
        ...
        earnings = [
            row
            for row in context.earnings_by_company.get(company.id, [])
        ...
        guidance_rows = [
            row
            for row in context.guidance_by_company.get(company.id, [])
            if row.accepted_for_scoring is True
            and (row.effective_session is None or row.effective_session <= context.cutoff)
            and (
                not request.from_session
        ...
        if guidance_rows:
            latest = max(
                guidance_rows, key=lambda row: (row.effective_session or date.min, row.id or 0)
            )
            derived_rows.append(
                self._derived_row(
                    company_id=company.id,
        ...
        for event, revision in current_catalysts:
            catalyst_values.append(
                _json_safe(
                    asdict(
                        self.catalysts.calculate(
    ...
    def _price_response_row(
        self,
        company: CeriCompany,
        context: CeriFeatureBatchContext,
        catalysts: list[tuple[CeriCatalystEvent, CeriCatalystEventRevision]],
    ) -> CeriPriceResponseFeature:
        ...
        *,
        ...
        if _is_postgresql(db):
            ...
            _execute_upsert(
                ...
                CeriPriceResponseFeature,
                [price_row],
                "uq_ceri_price_response_event_key",
        ...
        existing_price = next(
            (
                row
                for row in context.existing_price_features_by_company.get(price_row.company_id, [])
                if row.event_key == price_row.event_key
            ),
            None,
        )
    ...
    def _persistence_counts(
        ...
        pairs = [
            (row, context.existing_revision_features.get(_revision_key(row)))
            for row in revision_rows
        ]
        pairs.extend(
            (row, context.existing_derived_features.get(_derived_key(row))) for row in derived_rows
        )
        price_existing = next(
            (
                row
    ...
    def _input_fingerprint(
        ...
        rows.extend(
            context.source_records_by_id[source_id]
            for source_id in sorted(source_ids)
            if source_id in context.source_records_by_id
        )
        rows.extend(context.bars_by_ticker.get(company.ticker.upper(), []))
        rows.extend(context.bars_by_ticker.get(self.config.price_response.benchmark.upper(), []))
        return _stable_hash(
            {
                "company_id": company.id,
                "as_of_session": context.cutoff.isoformat(),
    ...
    def _existing_output_fingerprint(
        self, company_id: int, context: CeriFeatureBatchContext
    ) -> tuple[str, int]:
        rows: list[Any] = [
            row
    ...
    def _context_companies(
        self, context: CeriFeatureBatchContext, request: CeriFeatureRebuildRequest
    ) -> list[CeriCompany]:
    ...
    def _add_acceleration(
        self, calculated: list[CeriRevisionFeature], *, service: Any | None = None
    ) -> None:
        if len(calculated) < 2:
            return
...
_REVISION_UPDATE_COLUMNS = (
    ...
    "known_at",
    "reference_at",
    "revision_confidence_score",
    "revision_confidence_label",
    "warnings_json",
...
def _output_fingerprint(rows: Iterable[Any]) -> str:
    return _stable_hash(
        sorted(
            (
                row.__tablename__,
                getattr(row, "evidence_hash", None),
                getattr(row, "company_id", None),
                getattr(row, "event_key", None),
                getattr(row, "feature_family", None),
                getattr(row, "feature_key", None),
                getattr(row, "metric", None),
                getattr(row, "period_key", None),
                getattr(row, "window_days", None),
            )
            for row in rows
        )
    )
...
def _latest_price_event(
    ...
    *,
    ...
    if accepted:
        row = max(accepted, key=lambda item: (item.effective_session or date.min, item.id or 0))
        candidates.append(("GUIDANCE", row.id, row.effective_at, row.effective_session))
    catalyst_rows = (
        current_catalysts
        if current_catalysts is not None
        else _current_catalysts(db, company_id, cutoff)
    )
...
def _merge_results(results: list[CeriFeatureRebuildResult]) -> CeriFeatureRebuildResult:
    ...
    return CeriFeatureRebuildResult(
        ...
        warnings=sum(row.warnings for row in results),
        failed=sum(row.failed for row in results),
        errors=tuple(error for row in results for error in row.errors),
        short_circuited_families=tuple(
            sorted({family for row in results for family in row.short_circuited_families})
        ),
        family_runtime_ms=timings,
        family_query_counts=queries,
...
def _replace_result(result: CeriFeatureRebuildResult, **changes: Any) -> CeriFeatureRebuildResult:
