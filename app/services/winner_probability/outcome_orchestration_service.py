...
class H5NextOpenOrchestrationService:
    ...
    def drain_due(
        ...
        *,
        ...
        processed = matured = excluded = failed = target_stop_matured = 0
        ...
        full_drain_at = now.isoformat() if pending_after == 0 else None
        return H5DrainResult(
            due_h5_next_open=due_before,
            oldest_due_h5_session=oldest,
            oldest_due_h5_age=(
                us_trading_sessions_between(oldest, completed_on)
            ...
            last_successful_full_drain_at=full_drain_at,
        )
    ...
    def _count_unvisited(
        ...
        *,
        ...
        if callable(custom):
            return int(
                custom(db, completed_on=completed_on, exclude_ids=exclude_ids)
            )
        statement = (
