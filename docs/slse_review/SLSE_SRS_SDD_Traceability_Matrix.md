...
| SLSE-FR-010 | Exactly one current canonical snapshot per ticker/day | Canonical service and separate current pointer | `canonicalization.py` | Unique current-selection key | Snapshot timeline | Derived current label | canonical/schema/PostgreSQL concurrency tests | Deterministic single current pointer; historical decisions remain immutable | PASS | — |
...
| SLSE-FR-012 | Canonical revision audit without deletion | Pointer revision plus append-only selection event | canonicalization service | Current-selection pointer + selection-event ledger | Timeline | Partial | canonical/PostgreSQL tests | Prior snapshot remains byte-stable and pointer revision is audited | PASS | — |
...
| SLSE-NFR-011 | Append-only snapshots/source events | Append-only evidence plus explicit current indirection | repository/services | Snapshot decisions append; pointer mutates; pointer events append | N/A | N/A | schema/hash/concurrency tests | Implemented without historical demotion | PASS | — |
...
| AC-02 | Two same-day snapshots retained, one currently canonical | Canonical sequence | canonicalization | Unique current pointer | Timeline | Current and canonical-at-decision labels | canonical/PostgreSQL tests | Satisfied without rewriting either snapshot | PASS | — |
